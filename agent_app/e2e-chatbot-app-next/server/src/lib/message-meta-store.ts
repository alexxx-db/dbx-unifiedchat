/**
 * Best-effort in-memory store for message metadata (traceId, traceIds, chatId).
 *
...
const store = new Map<
  string,
  { traceId: string | null; traceIds: string[]; chatId: string }
>();
...
export const storeMessageMeta = (
  ...
  traceId: string | null,
  traceIds: string[] = [],
) => setBounded(store, messageId, { traceId, traceIds, chatId });
